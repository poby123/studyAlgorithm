...
int N, M, ans, map[300][300], temp[300][300], visit[300][300], mvy[] = {-1, 1, 0, 0}, mvx[] = {0, 0, -1, 1};
queue<pair<int, int>> q;

inline int isRange(const int &y, const int &x)
{
...
void check(int y, int x)
{
    ...
    while (!checkQ.empty())
    {
        auto front = checkQ.front();
        checkQ.pop();

        for (int d = 0; d < 4; d++)
        {
            int nextY = front.first + mvy[d];
            int nextX = front.second + mvx[d];

            if (isRange(nextY, nextX) && !visit[nextY][nextX] && map[nextY][nextX])
            {
                visit[nextY][nextX] = 1;
                checkQ.push({nextY, nextX});
            }
        }
    }
}
...
void melt()
{
    ...
    for (int i = 0; i < qSize; i++)
    {
        ...
        for (int d = 0; d < 4; d++)
        {
            int nextY = curY + mvy[d];
            int nextX = curX + mvx[d];
