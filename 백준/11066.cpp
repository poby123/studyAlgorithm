...
int T, K, A[MAX], DP[MAX][MAX], S[MAX];
...
int main()
{
    ...
    while (T--)
    {
        ...
        // 풀기
        for (int len = 1; len < K; len++) // 파일을 더할 길이
        {
            for (int i = 1; i <= K - len; i++) // 더할 시작점
            {
                int j = i + len; // 끝점
                DP[i][j] = INT_MAX;
                for (int mid = i; mid < j; mid++) // 2개로 분할할 중간점
                {
                    // DP[시작][끝] = min(DP[시작][끝], DP[시작][중간] + DP[중간][끝] + i~j까지의 누적 비용)
                    DP[i][j] = min(DP[i][j], DP[i][mid] + DP[mid + 1][j] + S[j] - S[i - 1]);
                }
            }
        }
