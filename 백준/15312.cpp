...
그래서 종민이는 알파벳 대문자로 이름을 써 놓고 이름 궁합을 보려고 한다. 그런데, 종민이는 손으로 계산을 하면 실수를 할까 두려워 당신에게 프로그램을 짜 달라고 부탁했다. 
종민이를 도와주자! 종민이가 정한 알파벳 대문자의 획수는 힌트를 참고하자.

입력
첫 번째 줄에는 종민이의 영어 이름 A가 주어진다. 
두 번째 줄에는 '그녀'의 영어 이름 B가 주어진다.
 A와 B 모두 알파벳 대문자로만 이루어진 길이 2 이상 2,000 이하의 문자열이며, 둘의 길이가 같음이 보장된다. 
...
int dp[MAX][MAX];
const int nums[26] = {3, 2, 1, 2, 3, 3, 2, 3, 3, 2, 2, 1, 2, 2, 1, 2, 2, 2, 1, 2, 1, 1, 1, 2, 2, 1};
...
int main(){
	...
	cout << dp[2*len-2][0] << dp[2*len-2][1] << "\n";
